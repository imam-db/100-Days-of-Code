command_list = ['off', 'report', 'espresso', 'latte', 'cappuccino']
MENU = {
    "espresso": {
        "ingredients": {
            "water": 50,
...
while is_on:
    command = input("What would you like? (espresso/latte/cappucino) ")
    ...
    if command == command_list[0]:
        ...
        if is_resources_sufficient(drink['ingredients']):
            ...
            if is_transaction_successfull(payment, drink['cost']):
                make_coffee(drink_name=command, order_ingredients=drink['ingredients'])